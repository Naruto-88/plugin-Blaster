...
export default function SitesPage() {
  ...
  const [sort, setSort] = useState<'createdAt'|'lastCheckedAt'|'name'|'url'|'status'|'severity'>('createdAt')
  const [order, setOrder] = useState<'asc'|'desc'>('desc')
  const [accum, setAccum] = useState<any[]>([])
  const { data, isLoading, isFetching } = trpc.sites.list.useQuery({ q, tags, statuses, page, pageSize, sort, order }, { keepPreviousData: true })
              ...
              {s.hasChangelog && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>Changelog available</TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )}
              {s._count?.logs ? (
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="text-[10px] rounded-full bg-zinc-200 dark:bg-zinc-800 px-2 py-0.5">{s._count.logs}</div>
                    </TooltipTrigger>
                    <TooltipContent>Logs count</TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ) : null}
            </div>
  ...
  // Top-right quick bulk actions (delete/clear) so users can find it easily
  const TopBulkBar = () => (
    selected.size > 0 ? (
      <div className="fixed top-20 right-4 z-50 card px-3 py-2 shadow-lg flex items-center gap-2">
        <div className="text-xs text-zinc-600">{selected.size} selected</div>
        <button
          disabled={role!=='admin'}
          className="rounded-lg border px-3 py-1 text-red-600 disabled:opacity-50"
          onClick={async ()=>{
            const msg = `You are about to permanently delete ${selected.size} site${selected.size>1?'s':''}.\n\nThis will remove their history (checks, plugin statuses, and logs).\nThis action cannot be undone.\n\nProceed?`
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <Dialog open={showImport} onOpenChange={setShowImport}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Import Sites (CSV / TXT)</DialogTitle>
              </DialogHeader>
              <ImportPanel onDone={()=>setShowImport(false)} />
            </DialogContent>
          </Dialog>
          <a className="text-xs underline" href="/sample-sites.csv" download>Sample CSV</a>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Select className="ml-auto w-[120px]" value={pageSize} onChange={e => setPageSize(parseInt(e.target.value))}>
